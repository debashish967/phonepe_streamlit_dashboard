...
if not os.path.exists(GEOFILE):
    st.error(f"{GEOFILE} not found in project folder. Place the geojson file here.")
    st.stop()
...
# -----------------------
# 3D Map (pydeck) — precompute colors & elevations to avoid JSON function calls
# -----------------------
...
# Build new geojson features with precomputed properties
geo_with_amount = {"type": "FeatureCollection", "features": []}
for feat in india_geo.get("features", []):
    props = dict(feat.get("properties", {}))
    dbn = props.get("db_name") or (props.get("NAME_1") or "").strip().lower()
    amt = float(amount_lookup.get(dbn, 0.0))
    tx = int(txn_lookup.get(dbn, 0))
    props["amount"] = amt
    props["transactions"] = tx
    props["display_name"] = dbn
    props["fill_color"] = amt_to_color_list(amt)
    # precompute elevation (scaled)
    # scale factor chosen to keep reasonable heights; tweak by dividing by bigger/smaller numbers
    props["elevation"] = float(amt) / (max_amt / 50000 + 1)  # heuristic
...
for _, row in df_map.iterrows():
    ...
    col_rows.append({
        ...
        "elev": float(amt) / (max_amt / 30000 + 1)
    })
...
deck = pdk.Deck(
    ...
    tooltip={"html": "<b>{properties.display_name}</b><br/>Amount: ₹{properties.amount:,.0f}<br/>Transactions: {properties.transactions}", "style": {"backgroundColor": "black", "color": "white"}},
